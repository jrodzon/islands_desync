...
class Ploter():
    ...
    def rysLine_z_tab100_dopliku(self, x, y, fileP, kt_wyspa, data_interval,title):  # tablice z X-ami i Y-ami
        kolorki = ['black','red','darkcyan','yellow','green','blue','cyan','tab:orange','tab:olive','tab:pink','tab:gray','tab:brown']
    ...
    def rysLine_z_tab_dopliku(self, x, y, fileP,kt_wyspa,title):
        ...
        plt.plot(x, y,color=kolorki[kt_wyspa+1])# , 'bs')  # t, t, 'r--', t, t ** 2, 'bs', t, t ** 3, 'g^') # todo: co to jest 'bs'
        plt.title(title)
        plt.savefig(fileP + '.png')
        plt.close()
    ...
    def rysLine_z_Ntab_dopliku(self, fileP, tabele,title,seria):
        ...
        if len(tabele[0])==3:
            for i in range(len(tabele)):
                ...
                plt.plot(tabele[i][0], tabele[i][1], color=kolorki[(i+1)% lk],linestyle=linestyle[nummarker],linewidth=1)
    ...
    def rysDot_z_Ntab_dopliku(self, fileP, tabele,title):
        ...
        for i in range(len(tabele)-1):
            plt.scatter(tabele[i+1][0],tabele[i+1][1], color=kolorki[i+1]) #,figsize=(25, 5))
        plt.title(title)
    ...
    def rysDot_z_tab_dopliku(self, xa, ya, x, y, fileP, numWyspy,title):
        #print("1-2D", fileP)
        kolorki = ['black', 'red', 'darkcyan', 'yellow', 'green', 'blue', 'cyan', 'tab:orange', 'tab:olive', 'tab:pink',
                   'tab:gray', 'tab:brown']
    ...
    def rysDot3d_z_Ntab_dopliku(self, tabele, fileP, title):
        ...
        ax.scatter(tabele[0][0], tabele[0][1], tabele[0][2], marker="*", color=kolorki[0])  # , edgecolor ="blue"

        #print("LT", len(tabele))
        for i in range(len(tabele) - 1):
            ax.scatter(tabele[i + 1][0], tabele[i + 1][1], tabele[i + 1][2], marker="o", c=kolorki[i + 1])
