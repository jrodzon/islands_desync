...
class BoxPloter():
    ...
    def makeIslandBoxPlot(self, folder,fileP,island,data_interval,title):
        ...
        df = pd.DataFrame(np.array(data_to_plot).T, columns=[str(data_interval*(x)) for x in range (len(jsonDat))])
        ...
        plt.show()
        plt.savefig(folder+"/fitness boxplotW"+str(island)+' '+str(data_interval)+'.png')
        plt.close()
