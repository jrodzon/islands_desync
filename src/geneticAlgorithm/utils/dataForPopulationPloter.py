...
class DataForPopulationPloter():
   ...
   def dataToPlotPopulation3D(self, katalogZCsv, ileplikow, ilewysp,dim, lowBound, upBound,
                              plot_population_interval):  # TODO: parametr - nazwa katalogu - wyberz csv-ki
      if self.czy_kom:
         print(" "+str(ileplikow)+" = "+str(ilewysp)+" * "+str(int(ileplikow/ilewysp)))
      ...
      for stk in range(int(ileplikow / ilewysp)):  # todo: potem dla 1-go pokolenia
         ...
         for wysp in range(ilewysp):
            ...
            with open(katalogZCsv + '/array step' + str(setek) + ' w' + str(wysp) + " dim"  +dim + ".csv", 'r') as file:  # todo !!!
            ...
            if (ilewysp > 1):# and wysp == kt_wyspa):
               self.plt.rysDot3d_z_tab_dopliku(tXbound,tYbound,tZbound,tX[wysp], tY[wysp],tZ[wysp],
                                        katalogZCsv + "/scatter" + str(setek) + " dim" + dim + " w" + str(
                                           wysp) , wysp,"Diversity of population in solution space on island "+str(wysp)) # todo bylo + " " + self.cz.czas()

         self.plt.rysDot3d_z_Ntab_dopliku(tabNWysp, katalogZCsv + "/scatter" + str(setek) + " dim" + dim, "Diversity of population in solution space on island ")
         #todo było: self.plt.rysDot3d_z_Ntab_dopliku(tabNWysp, katalogZCsv + "/scatter" + str(setek) + " dim" + dim + " " + self.cz.czas()


   def dataToPlotPopulation(self, katalogZCsv, ileplikow, ilewysp, dim, lowBound, upBound, plot_population_interval):
      if self.czy_kom:
         print(" "+str(ileplikow)+" = "+str(ilewysp)+" * "+str(int(ileplikow/ilewysp)))
      ...
      for stk in range(int(ileplikow/ilewysp)): #todo: potem dla 1-go pokolenia
         ...
         for wysp in range (ilewysp):
            ...
            if (ilewysp>1): # and wysp==kt_wyspa):
               self.plt.rysDot_z_tab_dopliku(tXbound,tYbound,tX[wysp], tY[wysp], katalogZCsv+"/scatter" + str(setek) +" dim"+dim+ " w"+str(wysp), wysp, "Diversity of population in solution space on island "+str(wysp))
               # todo było: +" " + self.cz.czas()

            tabNWysp.append([tX[wysp],tY[wysp]])

         self.plt.rysDot_z_Ntab_dopliku(katalogZCsv + "/scatter" + str(setek) + " dim" + dim,tabNWysp, "Diversity of population in solution space") #todo było:  + " " + self.cz.czas()
