...
class FilesLister():
    ...
    def listOfSubfolders(self, katalog): #, ext2like, ext3like): # przy przeszukiwaniu podkatalogów
        lista_elementow = os.listdir(katalog)
        properSubkat=[]
        for element in lista_elementow:
    ...
    def listOfSubfoldersEndingWith(self, katalog, ext1Like): #, ext2like, ext3like): # przy przeszukiwaniu podkatalogów
        lista_elementow = os.listdir(katalog)
        properSubkat=[]
        for element in lista_elementow:
    ...
    def listOfFilesNameConteining(self, katalog, ext2like): # przy przeszukiwaniu podkatalogów
    ...
    def listInsideSubfolders(self, katalog, properSubkat, ext2like, ext3like): # przy przeszukiwaniu podkatalogów testSB6

        listaOut = []
        for i in range(len(properSubkat)):
            listTmp = []
            listTmp.append(FilesLister.listFilesExtensionLike(0,katalog+"/"+properSubkat[i],ext3like))
            listaOut.append([elem  for elem in listTmp[0] if elem.__contains__(ext2like)])
        return listaOut
