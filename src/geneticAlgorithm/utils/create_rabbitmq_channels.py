        ...
        if self.number_of_islands > 1:
            ...
            if self.island == 0:
                print("\n\nW G_I_A ", self.number_of_islands, " WYSP\ndata interval", self.data_interval, "\nlast step: ",
                      self.last_step, ", max eval: ",
                      self.max_evaluations, ", population size: ", self.population_size,
                      ", offspring_population size: ", self.offspring_population_size, "\n\n")
            ...
            for i in range(0, self.number_of_islands):
                if i != self.island:
                    if self.rabbitmq_delays[str(self.island)][i] == -1:
                        continue
                    delay_channel.queue_bind(exchange='amq.direct',
