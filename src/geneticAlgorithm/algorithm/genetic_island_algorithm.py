...
class GeneticIslandAlgorithm(GeneticAlgorithm):
    def __init__(self,
        ...
        self.last_step = trunc(
            (self.termination_criterion.max_evaluations - self.population_size) / self.offspring_population_size)
        ...
        if self.problem.get_name()[0:4] == "Labs":  # <----       todo: LABS i problemy gdzie szukamy max
            self.Fname = self.fileName.getname(par_date + "_" + par_time, self.problem.get_name()[0:4],
                                               ...
                                               self.island, self.number_of_islands, self.population_size,
        ...
        self.path = self.fileName.getpath(self.par_date, self.problem.get_name()[0:4], self.problem.number_of_variables,
                                          self.par_time,
                                          self.number_of_islands, self.migrant_selection_type[0], "k",
                                          self.migration_interval, self.number_of_emigrants)
        ...
        if self.island == 0:
            ...
            if self.want_run_end_communications:
                print("\n\n\n                          The new directory is created! by island: " + str(
                    self.island) + "\n\n\n")
        else:
        ...
        self.logfile = logger.Logger(self.path + "/W" + str(self.island) + " log", self,
    ...
    def add_new_individuals(self):
        new_individuals, emigration_at_step_num = self.migration.receive_individuals(self.evaluations)
    ...
    def createCsvForThisStep(self, poprawa):  # OBRAZ GENERACJI W TYM MOMENCIE ZRZUT I RYSUNEK
        ...
        if poprawa:
            ...
            dataframeBig.to_csv(self.path + "/diversity-space/array step" + str(self.step_num) + " w" + str(
                self.island) + " dim_" + str(
                self.problem.number_of_variables) + ".csv")

    def createCsvWithTsne(self):
        fl = fileslister.FilesLister("qq", False)
        if self.want_tsne_to2 or self.want_tsne_to3:
            listOfFiles = fl.listFilesExtensionLike(self.path + "/diversity-space/",
    ...
    def saveXiYiFittnessWhileJump(self):
        self.tab_jump_best_result_and_all[self.tab_jump_ind] = {"stepX": self.step_num, "fitness": self.lastBest,
    ...
    def createEmigrJson(self):
        jsn = result_saver.Result_Saver(self.path + "/W" + str(self.island) + " Imigrants", self,
                                        self.want_run_end_communications)
        jsn.saveJson(self.tab_emigr)

    def createAllStepsDetailedPopulationJson(self):
        jsn = result_saver.Result_Saver(
            self.path + " - W" + str(self.island) + "every step population details-do step" + str(self.step_num), self,
            self.want_run_end_communications)
        jsn.saveJson(self.tab_detailed_population)
        self.tab_detailed_population = {}

    def createAllStepsResultsJson(self):
        jsn = result_saver.Result_Saver(self.path + "/resultsEveryStepW" + str(self.island), self,
                                        self.want_run_end_communications)
        jsn.saveJson(self.tab_all_steps_Y)

    def createJumpResultsJson(self):
        jsn = result_saver.Result_Saver(self.path + "/W" + str(self.island) + " results jump", self,
                                        self.want_run_end_communications)
        jsn.saveJson(self.tab_jump_best_result_and_all)

    def createJsonAllStepsDiversityJson(self):
        jsn = result_saver.Result_Saver(self.path + "/W" + str(self.island) + " set-minOS-srOS-diversity", self,
                                        self.want_run_end_communications)
    ...
    def createResultPlots(self):
        ...
        ploter.Ploter.rysLine_z_tab_dopliku(self, self.tab_X, self.tab_Y,
                                            self.path + "/results/resultW" + str(self.island), self.island,
    ...
    def createBoxplot(self):
        if self.want_create_boxplot:
            ...
            bp.makeIslandBoxPlot(self.path, "W" + str(self.island) + " results " + str(self.data_interval), self.island,
                                 self.data_interval, "Fitness diversity of island " + str(self.island))
    ...
    def writeSummaryResutlToConsoleAndFile(self):
        ...
        self.resultfile.writeLog("-" * 36 + "\nAverage result:  " + str(average)
                                 ...
                                 + " (this result was reached on : " + str(kt_ile) + "/" + str(
            self.number_of_islands) + " islands)", self)
        self.resultfile.saveLog()

        self.winnerfile.writeLog(str(kt), self)
        self.winnerfile.saveLog()

        print("\nmin result: ", minimal, "\n Winner Island:", kt, "\n(this result was reached on", kt_ile, "/",
    ...
    def step(self):
        ...
        if (self.step_num % self.plot_population_interval == 0 or self.step_num == 1):
            print(
                "step " + str(self.step_num) + " evaluations " + str(self.evaluations) + " island " + str(self.island))
            """if self.step_num != 1:
        ...
        self.solutions = self.replacement(self.solutions,
                                          offspring_population)  # todo !!! zobacz GŁĘBIEJ ten replacement
        # print("*********------------*******", self.solutions.__len__())
        ...
        if (self.last_step - 1 < self.step_num) and not self.bylLog:
            ...
            self.ctrl.endOfProcess(self.island,
                                   self.lastBest)  # tworzy plik END.ktrl - KONTROLNY Z WYNIKIEM Z TEJ WYSPY
