...
from utils import result_saver
from utils import boxPloter
from utils import fileslister
from utils import dataForPopulationPloter
...
class GeneticIslandAlgorithm(GeneticAlgorithm):
    def __init__(self,
                 ...
                 population_evaluator: Evaluator = store.default_evaluator) -> object:
        ...
        self.data_interval=round(self.termination_criterion.max_evaluations // self.how_many_data_intervals)
        ...
        self.last_step = trunc((self.termination_criterion.max_evaluations - self.population_size) / self.offspring_population_size)
        ...
        if self.number_of_islands>1:
            ...
            if self.island==0:
                print("\n\nW G_I_A ",self.number_of_islands," WYSP\ndata interval",self.data_interval, "\nlast step: ", self.last_step, ", max eval: ",
                      self.termination_criterion.max_evaluations, ", population size: ", self.population_size, ", offspring_population size: ", self.offspring_population_size, "\n\n")
            ...
            for i in range(0, self.number_of_islands):
                if i != self.island:
                    ...
                    self.delay_channel.queue_bind(exchange='amq.direct',
                                                  queue=f'island-from-{self.island}-to-{i}')  #WIECEJ NIZ 1 WYSPA #1-end
        ...
        if self.problem.get_name()[0:4]=="Labs":   #           <----       todo: LABS i problemy gdzie szukamy max
            self.Fname=self.fileName.getname(par_date+"_"+par_time,self.problem.get_name()[0:4],
                                             self.problem.number_of_variables,"",
                                             self.island,self.number_of_islands, self.population_size,self.offspring_population_size,
        ...
        self.path=self.fileName.getpath(self.par_date,self.problem.get_name()[0:4],self.problem.number_of_variables,self.par_time,
                                        self.number_of_islands, self.migrant_selection_type[0],"k",self.migration_interval,self.number_of_emigrants)
        ...
        if self.island==0:
            ...
            if self.want_run_end_communications:
                print("\n\n\n                          The new directory is created! by island: "+str(self.island)+"\n\n\n")
        else:
            while not (os.path.exists (self.path)):
                print("w"+str(self.island)+" waits")
                time.sleep(0.5)

        self.logfile = logger.Logger(self.path+ "/W"+str(self.island)+" log",self,self.want_run_end_communications)

        if self.island==0:
            self.resultfile=logger.Logger(self.path+"/___RESULT",self,self.want_run_end_communications)
            self.winnerfile = logger.Logger(self.path + "/___WINNER", self, self.want_run_end_communications)

        self.ctrl=controller.Controller(self.path, self.island,self.want_run_end_communications)
    ...
    def add_new_individuals(self):
        ...
        for i in range(0, 5):
            ...
            if body:
                ...
                self.tab_emigr[self.step_num] = {'step': self.step_num, 'ev': self.evaluations,
                            'fitn': data['objectives'][0], 'var':data['variables'] ,
                            'from_isl': data['from_island'], 'from_eval': data['from_evaluation']}
    ...
    def paramJson(self):
        ...
        jsn = result_saver.Result_Saver(self.path+"/param", self, self.want_run_end_communications)
        results = {"problem": self.problem.get_name(),
                   ...
                   "want_save_diversity_when_improvement": str(self.want_save_diversity_when_improvement),
        ...
        if self.number_of_islands >1:
            results["rabbitmq_delays: "]= str(self.rabbitmq_delays)
        jsn.saveJson(results)


    def uzupParamLog(self):   # dodaje info o krzyzowaniu, mutacji i selekcji z run_algorithm.py (tekst)
        douzup=""
        wlaczone=False
        file1 = open("run_algorithm.py", "r")
    ...
    def saveTabsAndRuningTimeInLogFile(self):
        self.logfile.writeLog("\n\nczas trwania: " + str(self.dta.teraz() - self.czasStart)+"\nlast step: "+str(self.step_num), self)
        self.bylLog = True


    # CSV SECTION  -----------------------------------------------------------
    def createCsvForThisStep(self,poprawa):    # OBRAZ GENERACJI W TYM MOMENCIE ZRZUT I RYSUNEK
        osobniki = []
        ...
        if poprawa:
            dataframeBig.to_csv(
                self.path + "/poprawa/poprawa step" + str(self.step_num) + " w" + str(self.island) + " dim__" + str(
                    self.problem.number_of_variables) + ".csv")
        else:
            #print("DF ",len(dataframeBig))
            dataframeBig.to_csv(self.path + "/diversity-space/array step" + str(self.step_num) + " w" + str(self.island) + " dim_" + str(
            self.problem.number_of_variables) + ".csv")
    ...
    def createCsvWithTsne(self):
        ...
        if self.want_tsne_to2 or self.want_tsne_to3:
            listOfFiles = fl.listFilesExtensionLike(self.path+"/diversity-space/", str(self.problem.number_of_variables)+".csv")
            tsneA = tsne.Tsne()
    ...
    def saveXiYiFittnessWhileJump(self):
        self.tab_jump_best_result_and_all[self.tab_jump_ind]={"stepX":self.step_num, "fitness":self.lastBest,"all_fitn":self.fitnessNow()}
        # self.tabelkaY.append(-self.lastBest) #todo for LABS
        self.tab_jump_ind+=1


    def saveDetailedPopulationDescriptionForThisStepInTab(self):    # OBRAZ GENERACJI W TYM MOMENCIE ZRZUT I RYSUNEK
        osobniki = []
        for solut in range(len(self.solutions)):
            lista = []
            for vari in range(self.solutions[solut].number_of_variables):
    ...
    def createEmigrJson(self):
        jsn = result_saver.Result_Saver(self.path+"/W"+str(self.island)+" Imigrants", self,self.want_run_end_communications)
        jsn.saveJson(self.tab_emigr)


    def createAllStepsDetailedPopulationJson(self):
        jsn = result_saver.Result_Saver(self.path+" - W"+str(self.island)+"every step population details-do step"+str(self.step_num), self,self.want_run_end_communications)
        jsn.saveJson(self.tab_detailed_population)
        self.tab_detailed_population = {}


    def createAllStepsResultsJson(self):
        jsn = result_saver.Result_Saver(self.path+"/resultsEveryStepW"+str(self.island), self,self.want_run_end_communications)
        jsn.saveJson(self.tab_all_steps_Y)


    def createJumpResultsJson(self):
        jsn = result_saver.Result_Saver(self.path+"/W"+str(self.island)+" results jump", self,self.want_run_end_communications)
        jsn.saveJson(self.tab_jump_best_result_and_all)


    def createJsonAllStepsDiversityJson(self):
        jsn = result_saver.Result_Saver(self.path+"/W"+str(self.island)+" set-minOS-srOS-diversity", self,self.want_run_end_communications)
    ...
    def createResultPlots(self):
        ...
        ploter.Ploter.rysLine_z_tab_dopliku(self, self.tab_X, self.tab_Y, self.path + "/results/resultW" + str(self.island), self.island, "Results of island " + str(self.island))


    def createBoxplot(self):
        if self.want_create_boxplot:
            bp = boxPloter.BoxPloter(self,self.want_run_end_communications)
            bp.makeIslandBoxPlot(self.path,"W"+str(self.island)+" results "+str(self.data_interval),self.island, self.data_interval, "Fitness diversity of island "+str(self.island))


    def createSpaceDiversityPopulationPlots(self):
        if (self.ctrl.isCtrlComplete(self.number_of_islands)and self.ctrl.isEndComplete(self.number_of_islands)):
    ...
    def writeSummaryResutlToConsoleAndFile(self):
        results = []
        for i in range(self.number_of_islands):
            with open(self.path + "/" + "kontrolW"+str(i)+"End.ctrl.txt", 'r') as file:
                data = file.readlines()
                for linia in data:
        ...
        self.resultfile.writeLog(       "-"*36+"\nAverage result:  " + str(average)
                                 + "\n"+"-"*36+"\nBest result   : " + str(minimal)+"\nWinner island: "+str(kt)
                                        + " (this result was reached on : "+str(kt_ile)+"/"+str(self.number_of_islands)+ " islands)", self)
        self.resultfile.saveLog()

        self.winnerfile.writeLog(str(kt),self)
        self.winnerfile.saveLog()

        print("\nmin result: ", minimal, "\n Winner Island:", kt, "\n(this result was reached on", kt_ile, "/", self.number_of_islands,
              "islands)")
    ...
    def step(self):
        ...
        if (self.step_num%self.plot_population_interval == 0 or self.step_num == 1) :
            print("step " + str(self.step_num) + " evaluations " + str(self.evaluations) + " island " + str(self.island))
            """if self.step_num != 1:
        ...
        self.solutions = self.replacement(self.solutions, offspring_population)  # todo !!! zobacz GŁĘBIEJ ten replacement
        #print("*********------------*******", self.solutions.__len__())
        ...
        self.tab_all_steps_Y[self.step_num] = self.lastBest             # BEST FITNESS DLA KAŻDEGO KROKU
        ...
        if (self.last_step-1< self.step_num) and not self.bylLog:
            ...
            self.ctrl.endOfProcess(self.island, self.lastBest) # tworzy plik END.ktrl - KONTROLNY Z WYNIKIEM Z TEJ WYSPY
