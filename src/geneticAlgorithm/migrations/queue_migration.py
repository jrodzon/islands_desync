...
from src.geneticAlgorithm.migrations.Migration import Migration
from src.geneticAlgorithm.solution.float_island_solution import FloatIslandSolution
...
class QueueMigration(Migration):
    ...
    def migrate_individuals(self, individuals_to_migrate):
        for i in individuals_to_migrate:
            destination = random.choice([i for i in range(0, self.number_of_islands) if
                                         i != self.island and self.rabbitmq_delays[str(self.island)][i] != -1])
    ...
    def receive_individuals(self, step_num: int, evaluations: int) -> (List, Dict | None):
        ...
        for i in range(0, 5):
            ...
            if body:
                ...
                emigration_at_step_num = {'step': step_num, 'ev': evaluations,
                ...
                float_solution = FloatIslandSolution(data['lower_bound'], data['upper_bound'],
                                                     ...
                                                     constraints=data['constraints'], variables=data['variables'],
